...
export default function ProductNotificationsPage() {
  ...
  useEffect(() => {
    console.log('Product notifications from context:', allNotifications);
  }, [allNotifications]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log('Filtered product notifications:', filtered);
    } catch (err) {
      console.error('Error filtering product notifications:', err);
      setLocalError('Error processing product notifications data');
    }
  ...
  const handleMarkAsRead = async (notificationId) => {
    ...
    try {
      console.log(`Marking product notification ${notificationId} as read`);
      const result = await markAsRead(notificationId);
      if (!result) {
        setLocalError('Failed to mark product notification as read. Please try again.');
      } else {
        console.log(`Successfully marked product notification ${notificationId} as read`);
      }
    } catch (error) {
  ...
  const handleMarkAllAsRead = async () => {
    try {
      console.log('Marking all product notifications as read');
      const result = await markAllAsRead();
      if (!result) {
        setLocalError('Failed to mark all product notifications as read. Please try again.');
      } else {
        console.log('Successfully marked all product notifications as read');
      }
    } catch (error) {
      console.error('Error marking all product notifications as read:', error);
      setLocalError('Failed to mark all product notifications as read. Please try again.');
    }
  };
  ...
  const handleRetry = () => {
    console.log('Refreshing product notifications...');
    setLocalError(null);
    retryFetch();
  };
  ...
  return (
    <AdminLayout title="Product Notifications">
      <Head>
        <title>Product Notifications | Rangya</title>
      </Head>
        ...
        <div className="divide-y divide-gray-200">
          {loading ? (
            <div className="py-12 text-center text-gray-500">
              ...
              <p>Loading product notifications...</p>
            </div>
          ) : displayError ? (
            ...
            <div className="py-12 text-center text-gray-500">
              <p>No product notifications found</p>
              <button 
                onClick={handleRetry}
                    ...
                    <tr key={notification.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                          ${notification.type === 'low_stock' ? 'bg-yellow-100 text-yellow-800' : 
                            notification.type === 'out_of_stock' ? 'bg-red-100 text-red-800' : 
                            notification.type === 'new_product' ? 'bg-green-100 text-green-800' : 
